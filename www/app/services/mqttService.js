 ...
 *
 * Thanks to denschu from github.com
 */

var mqttServiceModule = angular.module("homeui.mqttServiceModule", ['ngResource']);
...
mqttServiceModule.factory('MQTTClient', function($rootScope) {
    ...
    service.onConnect = function() {
        console.log("Connected to " + client.host + ":" +
            client.port + " as '" + client.clientId + "'");
        client.subscribe("/devices/#");
    };

    service.onFailure = function() {
        console.log("Failure to connect to " + client.host + ":" +
            client.port + " as " + client.clientId);
    };
    ...
    service.onMessageArrived = function(message) {
        console.log("Arrived message: " + message.destinationName +
            " with " + message.payloadBytes.length + " bytes of payload");
